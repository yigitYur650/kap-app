...
- [x] **T-07**: Veri Normalizasyonu & Infinite Loop ve Performans İyileştirmeleri (MANDATORY Architecture Fixes)
  - Markets tablosu entegrasyonu ve `_getOrCreateMarketId` ile market normalizasyonu yapıldı.
  - Ürün eklemede `is_deleted = false` durumu insert payload'una açıkça eklenerek NULL filtreleme hatası önlendi.
  - Stream getter'larındaki `Timer.run(...)` çağrıları tamamen kaldırılarak Oppo Reno 13F gibi yüksek performanslı cihazlardaki sonsuz döngü (infinite rebuild) ve bellek sızıntıları giderildi.
  - Tüm Supabase DDL ve veri yazma operasyonlarına 10 saniyelik `.timeout()` eklendi.
  - Getter akışları `async*` üreteç (generator) akışlarına dönüştürülerek, geç abone olan (`StreamBuilder`) arayüz bileşenlerinin yükleme ekranında takılı kalmasını önleyecek şekilde aktif önbellek durumunu ilk adımda sunması (`yield`) sağlandı (BehaviorSubject yapısı taklit edildi).
